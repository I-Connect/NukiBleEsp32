...
void NukiBle::runStateMachine() {
  switch (nukiState) {
    case NukiState::startUp : {
      if ( (preferences.getBytes("secretKeyK", secretKeyK, 32) > 0) && (preferences.getBytes("authorizationId", authorizationId, 4) > 0) ) {
        nukiState = NukiState::paired;
        #ifdef DEBUG_NUKI
    ...
    case NukiState::startPairing : {
      ...
      if (millis() - timeNow > timeWait10Sec) {
        timeNow = millis();
    ...
    case NukiState::pairing : {
      uint8_t connectState = connectStateMachine();
      if (connectState == 1) {
        saveCredentials();
        nukiState = NukiState::paired;
      } else if (connectState == 0) {
        #ifdef DEBUG_NUKI
        log_d("Connect FAILED at connect state: %d ", nukiConnectState);
        #endif
      }
      break;
    }
    case NukiState::paired : {
      log_d("1");
...
void NukiBle::saveCredentials() {
  preferences.putBytes("secretKeyK", secretKeyK, 32);
  preferences.putBytes("authorizationId", authorizationId, 4);
  #ifdef DEBUG_NUKI
...
uint8_t NukiBle::connectStateMachine() {

  switch (nukiConnectState)
  {
    case NukiConnectState::initConnect: {
      #ifdef DEBUG_NUKI
      ...
      receivedStatus = 0xff;
      nukiConnectState = NukiConnectState::reqRemPubKey;
      break;
    }
    case NukiConnectState::reqRemPubKey: {
      ...
      timeNow = millis();
      nukiConnectState = NukiConnectState::recRemPubKey;
      break;
    }
    case NukiConnectState::recRemPubKey: {
      if (checkCharArrayEmpty(remotePublicKey, sizeof(remotePublicKey))) {
        nukiConnectState = NukiConnectState::sendPubKey;
      } else if (millis() - timeNow > timeWait10Sec) {
        log_w("Remote public key receive timeout");
        return false;
      }
      break;
    }
    case NukiConnectState::sendPubKey: {
      #ifdef DEBUG_NUKI
      ...
      nukiConnectState = NukiConnectState::genKeyPair;
      break;
    }
    case NukiConnectState::genKeyPair: {
      #ifdef DEBUG_NUKI
      ...
      timeNow = millis();
      nukiConnectState = NukiConnectState::calculateAuth;
      break;
    }
    case NukiConnectState::calculateAuth: {
      if (checkCharArrayEmpty(challengeNonceK, sizeof(challengeNonceK))) {
        ...
        memset(challengeNonceK, 0, sizeof(challengeNonceK));
        nukiConnectState = NukiConnectState::sendAuth;
      } else if (millis() - timeNow > timeWait10Sec) {
    ...
    case NukiConnectState::sendAuthData: {
      if (checkCharArrayEmpty(challengeNonceK, sizeof(challengeNonceK))) {
        ...
        timeNow = millis();
        nukiConnectState = NukiConnectState::sendAuthIdConf;
      } else if (millis() - timeNow > timeWait10Sec) {
    ...
    case NukiConnectState::sendAuthIdConf: {
      if (checkCharArrayEmpty(authorizationId, sizeof(authorizationId))) {
        ...
        timeNow = millis();
        nukiConnectState = NukiConnectState::recStatus;
      } else if (millis() - timeNow > timeWait10Sec) {
    ...
    case NukiConnectState::recStatus: {
      if (receivedStatus == 0) {
        #ifdef DEBUG_NUKI
        log_d("####################### CONNECT DONE ###############################################");
        #endif
